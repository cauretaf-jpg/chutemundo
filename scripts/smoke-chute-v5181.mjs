...
try {
  ...
  if (summary.version !== '5.18.1' || summary.title !== 'Estadísticas' || summary.subtitle !== 'Equipos, jugadores, torneos y récords.') throw new Error(`Encabezado incorrecto: ${JSON.stringify(summary)}`);
  ...
  await page.waitForFunction(() => {
    ...
    return document.getElementById('estadisticas')?.classList.contains('cm-v5181-analysis-open') && root && !root.hidden && root.getClientRects().length > 0;
  });
  ...
  if (analysis.rootDisplay === 'none' || analysis.filters !== 6 || !analysis.contentHidden || !analysis.toolbarFiltersHidden || !analysis.active || !analysis.rootText.includes('Análisis histórico')) throw new Error(`Análisis histórico no abrió correctamente: ${JSON.stringify(analysis)}`);
  ...
  await page.waitForFunction(() => {
    ...
    const root = document.getElementById('cmV58AnalysisRoot');
    return !pageElement?.classList.contains('cm-v5181-analysis-open') && root?.hidden && document.querySelector('#cmV518Stats .cm-v518-content')?.hidden === false;
  });
  ...
  if (mobile.width > mobile.viewport + 3 || !mobile.title.includes('5.18.1')) throw new Error(`Vista móvil o versión incorrecta: ${JSON.stringify(mobile)}`);
  ...
  console.log('Chute Mundo v5.18.1 smoke OK', { summary, analysis, mobile });
} finally {
  await browser.close();
}
