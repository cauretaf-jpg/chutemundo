...
try{
  ...
  const result=await page.evaluate(()=>{
    ...
    copy.id='tour_smoke_stable';copy.status='upcoming';copy.matches=(copy.matches||[]).map(m=>({...m,homeGoals:null,awayGoals:null,goals:[],cards:[]}));state.tournaments.push(copy);
  ...
  if(result.core!=='5.11.0'||result.tournaments!=='5.11.0'||result.share!=='5.11.0'||result.integrity!=='5.12.0'||!result.sameTeams||result.started!=='active'||result.search<1||!result.issues||!result.manifest||!result.toolbar||result.width>result.viewport+3)throw new Error(JSON.stringify(result));
  console.log('Chute Mundo v5.12 stable smoke OK',result);
