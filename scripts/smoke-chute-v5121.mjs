...
try{
  ...
  if(!result.title.includes('5.12.1')||result.coreVersion!=='5.11.1'||result.searchVersion!=='5.12.1'||result.backupVersion!=='5.12.1'||!result.playerId.includes('__')||result.playerId.includes('::')||result.champion!=='a'||result.runnerUp!=='c'||result.width>result.viewport+3)throw new Error(JSON.stringify(result));
  console.log('Chute Mundo v5.12.1 smoke OK',result);
