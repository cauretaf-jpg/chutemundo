...
const nativeServiceWorkerRegister = 'serviceWorker' in navigator ? navigator.serviceWorker.register.bind(navigator.serviceWorker) : null;
const nativeServiceWorkerAddEventListener = 'serviceWorker' in navigator ? navigator.serviceWorker.addEventListener.bind(navigator.serviceWorker) : null;
...
function installCanonicalVersionLock() {
  ...
  } catch (error) { console.warn('No se pudo bloquear el título global.', error); }
  ...
  try {
    Object.defineProperty(heroVersion, 'textContent', {
      ...
      set: (value) => {
        ...
        if (!/^CHUTE MUNDO v\d/i.test(text) || text === canonicalHero) nodeTextDescriptor.set.call(heroVersion, text === canonicalHero ? canonicalHero : text);
        else console.info(`Se ignoró un encabezado heredado: ${text}`);
      }
    });
    heroVersion.replaceChildren = (...nodes) => {
      const text = nodes.map((node) => typeof node === 'string' ? node : node?.textContent || '').join('');
      if (/^CHUTE MUNDO v\d/i.test(text) && text !== canonicalHero) { console.info(`Se ignoró un encabezado heredado: ${text}`); return; }
      nativeReplaceChildren(...nodes);
    };
  } catch (error) { console.warn('No se pudo bloquear el encabezado de versión.', error); }
}
...
function installServiceWorkerListenerGuard() {
  ...
  const guardedAddEventListener = (type, listener, options) => {
    if (type === 'controllerchange') { console.info('Se ignoró una recarga heredada por cambio de service worker.'); return; }
...
function workerVersion(registration) {
  ...
  if (!scriptUrl) return '';
  try { return new URL(scriptUrl).searchParams.get('v') || ''; } catch { return ''; }
}
...
function waitForActivation(registration, timeout = 8000) {
  ...
  return new Promise((resolve) => {
    ...
    const check = () => { if (worker.state === 'activated' || registration.active?.state === 'activated' || worker.state === 'redundant') finish(); };
...
function installServiceWorkerRegistrationGuard() {
  ...
  const guardedRegister = (scriptURL, options = {}) => {
    let requestedVersion = ''; let requestedPath = '';
    try { const requested = new URL(String(scriptURL), window.location.href); requestedVersion = requested.searchParams.get('v') || ''; requestedPath = requested.pathname; } catch {}
    if (requestedPath.endsWith('/sw.js') && requestedVersion !== APP_VERSION) { console.info(`Se ignoró un registro PWA heredado (${requestedVersion || 'sin versión'}).`); return registerCurrentWorker(); }
...
async function resetLegacyRuntimeOnce() {
  if (LOCAL_HOSTS.has(window.location.hostname)) { removeResetQuery(); return; }
  ...
  try {
    ...
    await registerCurrentWorker(); cleaned = true;
  } catch (error) { console.warn('La limpieza automática de caché no pudo completarse.', error); }
...
function restoreHomeAfterBoot(coreInstance) { if (!userNavigated && coreInstance?.navigate) coreInstance.navigate('inicio'); }
document.addEventListener('click', (event) => { if (event.target.closest?.('[data-page],[data-cm-page],[data-cm-mobile-page]')) userNavigated = true; }, true);
document.addEventListener('chute:boot-complete', (event) => { bootCompleted = true; applyVersion(); restoreHomeAfterBoot(event.detail?.core || window.ChuteMundoCore); }, { once: true });
...
window.ChuteVersion = Object.freeze({ version: APP_VERSION, title: APP_TITLE, cacheName: CACHE_NAME, apply: applyVersion, registerCurrentWorker, get bootCompleted() { return bootCompleted; } });
