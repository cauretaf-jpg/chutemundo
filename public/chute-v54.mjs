...
function automaticComposition() {
  ...
  if (previous) {
    if (!previous.nextComposition?.first?.length || !previous.nextComposition?.second?.length) {
      return { blocked: `La temporada “${previous.name}” todavía no define ascenso y descenso.`, source: 'Temporada anterior pendiente', first: [], second: [] };
    }
...
function divisionMatches(teamIds, legs, divisionName, divisionKey) {
  ...
  const matches = [];
  const append = (rounds, offset, reverse = false) => rounds.forEach((pairs, roundIndex) => pairs.forEach((pair, matchIndex) => {
    matches.push(newMatch({ id: uid(`ds_${divisionKey}`), stage: 'group', group: divisionName, round: `Fecha ${offset + roundIndex + 1}`, label: `Partido ${matchIndex + 1}`, home: reverse ? pair.away : pair.home, away: reverse ? pair.home : pair.away }));
  }));
  append(base, 0, false);
...
function buildSeason({ name, status = 'upcoming', legs = 2, firstPlayoff = true, secondPlayoff = true, finalLegs = 1, promotionMode = 'playoff', firstIds, secondIds }) {
  ...
  const tournament = {
    id: uid('season'), name, type: TYPE, era: 'division', status, createdAt: new Date().toLocaleDateString('es-CL'),
    config: {
    ...
    notes: ['Primera y Segunda División pertenecen a una misma temporada.', 'El último de Primera desciende y el ascendido de Segunda se incorpora automáticamente a la temporada siguiente.'],
    playerScorers: [], playerAssists: [], divisionResults: null, nextComposition: null, champion: null, runnerUp: null, third: null,
    participantLocal: 'participante_alvaro', participantAway: 'participante_carlos', participantChampion: '', participantRunnerUp: '', participantThird: ''
  };
  return tournament;
}
...
function standings(tournament, divisionName) {
  ...
  const rows = Object.fromEntries((group?.teamIds || []).map((teamId) => [teamId, { teamId, pj: 0, pg: 0, pe: 0, pp: 0, gf: 0, gc: 0, dg: 0, pts: 0 }]));
  ...
  return Object.values(rows).map((row) => ({ ...row, dg: row.gf - row.gc })).sort((a, b) => b.pts - a.pts || b.dg - a.dg || b.gf - a.gf || a.gc - b.gc || teamName(a.teamId).localeCompare(teamName(b.teamId), 'es')).map((row, index) => ({ ...row, pos: index + 1 }));
}
...
function createPlayoff(tournament, divisionName, divisionKey, rows) {
  const legs = Number(tournament.config?.finalLegs || 1);
  const leader = rows[0]?.teamId; const runner = rows[1]?.teamId;
  if (!leader || !runner || playoffMatches(tournament, divisionName).length) return false;
  if (legs === 2) {
    tournament.matches.push(
      newMatch({ id: `${tournament.id}_${divisionKey}_final_1`, stage: 'knockout', round: 'Play-Off', label: `Final ${divisionName} · Ida`, group: divisionName, home: runner, away: leader }),
      newMatch({ id: `${tournament.id}_${divisionKey}_final_2`, stage: 'knockout', round: 'Play-Off', label: `Final ${divisionName} · Vuelta`, group: divisionName, home: leader, away: runner })
    );
...
function finalOutcome(tournament, divisionName) {
  ...
  if (matches.length === 1) {
    const winner = matchWinner(matches[0]);
    return winner ? { winner, runnerUp: winner === matches[0].home ? matches[0].away : matches[0].home } : null;
  }
  const totals = new Map();
  for (const match of matches) {
    totals.set(match.home, (totals.get(match.home) || 0) + Number(match.homeGoals));
    totals.set(match.away, (totals.get(match.away) || 0) + Number(match.awayGoals));
  }
  ...
  if (!winner) {
    const secondLeg = matches.at(-1);
    if (secondLeg.homePens !== null && secondLeg.awayPens !== null && Number(secondLeg.homePens) !== Number(secondLeg.awayPens)) winner = Number(secondLeg.homePens) > Number(secondLeg.awayPens) ? secondLeg.home : secondLeg.away;
  }
...
function syncTournament(tournament) {
  ...
  if (firstDone && secondDone && firstFinal?.winner && secondFinal?.winner && promoted && relegated) {
    ...
    if (signature !== previousSignature) {
      tournament.divisionResults = outcome;
      tournament.nextComposition = { first: nextFirst, second: nextSecond, sourceTournamentId: tournament.id };
      tournament.champion = firstFinal.winner; tournament.runnerUp = firstFinal.runnerUp;
      tournament.third = firstRows.find((row) => ![firstFinal.winner, firstFinal.runnerUp].includes(row.teamId))?.teamId || null;
      tournament.status = 'historical';
      tournament.notes = Array.from(new Set([...(tournament.notes || []), `${teamName(promoted)} asciende a Primera División.`, `${teamName(relegated)} desciende a Segunda División.`]));
      changed = true;
    }
  }
  return changed;
...
function enhanceDivisionHub() {
  ...
  let seasonInfo = hub.querySelector('.cm-v54-season-info');
  if (!seasonInfo) { seasonInfo = document.createElement('section'); seasonInfo.className = 'cm-v54-season-info'; hub.querySelector('.cm-hub-heading')?.insertAdjacentElement('afterend', seasonInfo); }
  const result = tournament.divisionResults;
  seasonInfo.innerHTML = `<article><span>Primera División</span><b>${tournament.config?.playoffs?.first ? 'Con Play-Off' : 'Tabla regular'}</b></article><article><span>Segunda División</span><b>${tournament.config?.playoffs?.second ? 'Con Play-Off' : 'Tabla regular'}</b></article><article><span>Ascenso</span><b>${tournament.config?.promotionMode === 'playoff' ? 'Ganador Play-Off' : 'Líder regular'}</b></article><article><span>Formato</span><b>${Number(tournament.config?.legs || 1) === 2 ? 'Ida y vuelta' : 'Solo ida'}</b></article>${result ? `<div class="cm-v54-movement"><strong>↑ ${esc(teamName(result.promoted))} asciende</strong><strong>↓ ${esc(teamName(result.relegated))} desciende</strong></div>` : ''}`;
}
...
function enhanceCompactFixture() {
  ...
  fixture.querySelectorAll('.cm-hub-match').forEach((card) => {
    if (card.dataset.cmCompact === 'true') return;
    card.dataset.cmCompact = 'true'; card.classList.add('cm-v54-compact-match');
    const header = card.querySelector(':scope > header');
    if (header) { const button = document.createElement('button'); button.type = 'button'; button.className = 'cm-v54-match-toggle'; button.setAttribute('aria-expanded', 'false'); button.innerHTML = '<span>Detalle</span><b>⌄</b>'; header.appendChild(button); }
  });
}
...
document.addEventListener('change', (event) => {
  if (event.target.id === 'tournamentType') { stateCache.composition = null; window.setTimeout(ensureFormUI, 0); return; }
  if (event.target.closest('[data-cm-division-slot]')) stateCache.composition = readCompositionFromUI();
  if (event.target.id === 'cmSecondPlayoff') { const promotion = document.getElementById('cmPromotionMode'); if (promotion) { promotion.disabled = !event.target.checked; if (!event.target.checked) promotion.value = 'regular'; } }
  window.setTimeout(() => syncAll(), 650);
});
document.addEventListener('click', (event) => {
  if (event.target.closest('[data-cm-reset-composition]')) { stateCache.composition = null; stateCache.source = ''; ensureFormUI(); return; }
  const toggle = event.target.closest('.cm-v54-match-toggle');
  if (toggle) {
    event.preventDefault(); event.stopPropagation();
    const card = toggle.closest('.cm-hub-match'); const fixture = card?.closest('[data-cm-tournament-panel="fixture"]'); const open = !card.classList.contains('expanded');
    fixture?.querySelectorAll('.cm-hub-match.expanded').forEach((item) => { item.classList.remove('expanded'); item.querySelector('.cm-v54-match-toggle')?.setAttribute('aria-expanded', 'false'); });
    card?.classList.toggle('expanded', open); toggle.setAttribute('aria-expanded', String(open));
  }
  window.setTimeout(() => syncAll(), 750);
});
document.addEventListener('submit', () => window.setTimeout(() => syncAll(), 800));
refresh();
window.setInterval(() => { refresh(); syncAll(); }, 1100);
window.ChuteDivisionsV54 = { TYPE, FIRST, SECOND, fifaOrder, automaticComposition, buildSeason, standings, syncTournament, syncAll, refresh };
